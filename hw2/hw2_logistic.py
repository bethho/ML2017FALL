...
def load_data(train_data_path, train_label_path, test_data_path, A, k):
    ...
    for i in range(k-1):
        q = i+2;
        X_train = np.concatenate((X_train,X_train**q), axis=1)
        X_test = np.concatenate((X_test,X_test**q), axis=1)
...
def main(opts):
    ...
    X_all, Y_all, X_test = load_data(opts.train_data_path, opts.train_label_path, opts.test_data_path, A, k)
    # Normalization
