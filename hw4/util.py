...
class DataManager:
    ...
    def add_data(self,name, data_path, with_label=True):
        ...
        if with_label:
            ...
            self.data[name] = [X]  # Build dictionary

    def add_testdata(self, name, data_path, with_label=True):
        print('read data from %s...' % data_path)
        X, Y = [], []
        i = 0
        with open(data_path, 'r') as f:
    ...
    def tokenize(self, vocab_size, del_symbol = 'True'):
        if del_symbol:
            fils = '!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n'
        else:
            fils = ''
        print('create new tokenizer')
        self.tokenizer = Tokenizer(num_words=vocab_size,filters=fils)
